...
void	cam_rotate_up(t_scene *sc)
{
	...
	sc->cam.orient = vec_normalize(rotate_vector(sc->cam.orient, right, sc->cam_r_speed));
	sc->render = true;
}
...
void	cam_rotate_left(t_scene *sc)
{
	...
	sc->cam.orient = vec_normalize(rotate_vector(sc->cam.orient, (t_vec4){0, 1, 0}, -sc->cam_r_speed));
	sc->render = true;
}

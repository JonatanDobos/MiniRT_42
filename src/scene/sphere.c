...
// // 		{
// // 			float dist = sqrt(pow(x - WINDOW_WIDTH / 2, 2) + pow(y - WINDOW_HEIGHT / 2, 2));
...
// // 	rt.mlx = mlx_init(WINDOW_WIDTH, WINDOW_HEIGHT, "Sphere Showcase", true);
// // 	if (!rt.mlx)
// // 	{
...
// int intersect_sphere(const t_sphere *sphere, const t_vec *coords, const t_vec *ray_pos)
// {
// 	t_vec oc = {
...
// 	double a = dot(coords, coords);
// 	double b = 2.0 * dot(&oc, coords);
...
// 				.z = base_ray_dir->z
// 			};
// 			coords = snormalize(coords);

// 			if (intersect_sphere(sphere, &coords, ray_pos))
// 			{

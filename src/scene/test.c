...
double intersect_plane(t_objs *obj, const t_vec3 ray_dir, const t_vec3 ray_pos)
{
    t_plane *plane = &obj->plane;
    double intersection_distance;
    double denom = dot_product(plane->direction, ray_dir); // Using dot product
    if (fabs(denom) > 1e-6) {
...
double intersect_sphere(t_objs *obj, const t_vec3 ray_dir, const t_vec3 ray_pos)
{
	double radius = obj->sphere.radius;
	t_vec3 oc = ray_pos - obj->coords; // Using vector subtraction

	double a = dot_product(ray_dir, ray_dir); // Using dot product
	double b = 2.0 * dot_product(oc, ray_dir); // Using dot product
	double c = dot_product(oc, oc) - radius * radius; // Using dot product
...
{
	static double (*intersect_obj[NUM_OBJ_TYPES])(t_objs *, const t_vec3, const t_vec3) = {
		intersect_plane,
		intersect_sphere
	};

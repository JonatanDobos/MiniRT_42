...
// bool intersect_plane(const t_plane *plane, const t_vec *ray_direction, const t_vec *ray_pos, double *t)
// {
// 	t_vec p0l0 = {
...
void render_plane(mlx_image_t *img, const t_plane *plane, const t_vp *vp)
{
	...
	// const t_vec	*ray_dir = &vp->ray_direction;
	...
	for (int j = 0; j < HEIGHT; j++)
	{
		for (int i = 0; i < WIDTH; i++)
		{
			...
			double t;
			// if (intersect_plane(plane, &ray_direction, ray_pos, &t))
			if (intersect_plane(plane, &ray_direction, ray_pos, &t))
			{

...
void	ambient_line(t_dbltoa *dbl, t_objs *ambient)
{
...
void	camera_line(t_dbltoa *dbl, t_objs *camera)
{
	char	rt_line[RT_MAX_LINE_LEN];
	size_t	line_index;
...
void	lights_line(t_dbltoa *dbl, t_objs *lights, size_t amount)
{
	char	rt_line[RT_MAX_LINE_LEN];
	size_t	line_index;
	size_t	i;
...
void	create_scene_rt_file(t_scene *sc)
{
	...
	ambient_line(&dbl, &sc->ambient);
	camera_line(&dbl, &sc->camera);
	lights_line(&dbl, sc->lights, sc->l_arr_size);

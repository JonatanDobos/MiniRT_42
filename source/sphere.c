...
int intersect_sphere(const t_sphere *sphere, const t_vec *ray_direction, const t_vec *ray_origin)
{
    t_vec oc = {
        .x = ray_origin->x - sphere->center.x,
        .y = ray_origin->y - sphere->center.y,
        .z = ray_origin->z - sphere->center.z
    };
...
void render_sphere(mlx_image_t *img, const t_sphere *sphere, const t_vec *ray_origin)
{
    ...
    for (int j = 0; j < HEIGHT; j++)
    {
        ...
        {
            t_vec ray_direction = {
                .x = (2.0 * ((i + 0.5) / WIDTH) - 1.0) * aspect_ratio,
                .y = 1.0 - 2.0 * ((j + 0.5) / HEIGHT),
                .z = -1.0
            };
            ray_direction = snormalize(ray_direction);

            if (intersect_sphere(sphere, &ray_direction, ray_origin))
            {
...
t_sphere sphere(t_rt *rt)
{
    ...
    render_sphere(rt->img, &sphere, &rt->ray_origin);
    return sphere;
}